...
from utils.tools import del_files, EarlyStopping, adjust_learning_rate, vali, load_content, test_classification , validation_classification
...
for epoch in range(args.train_epochs):
    ...
    pbar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.train_epochs}", leave=False)

    for batch_x, batch_y, batch_x_mark, batch_y_mark, report in pbar:
        ...
        if 'classification' in args.task_name:
            ...
            batch_y_true = batch_y[:, -args.pred_len:, f_dim:]
            loss = nn.MSELoss()(outputs, batch_y_true)
    ...
    if 'classification' in args.task_name:
        ...
        accelerator.print(
            ...
            f"Test Loss: {test_loss:.7f} Acc: {test_acc:.4f}"
        )
    else:
