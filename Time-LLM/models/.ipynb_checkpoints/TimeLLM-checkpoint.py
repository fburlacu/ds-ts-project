...
###############################          NEW          #############################
    def _build_prompt(self, min_val_str, max_val_str, median_val_str,
                  trend_str, lags_str, report=""):
        """
        Build the Prompt-as-Prefix string for a single sample.
    
        Args:
            x_b   : tensor of shape (T, C) — the raw input for one batch item
            report: optional doctor's clinical note for this sample (may be "")
        
        """
        if self.task_name == 'classification':
            task_desc = (
                f"classify whether this recording is normal or abnormal "
                f"given the previous {str(self.seq_len)} timesteps of input"
            )
        else:
            task_desc = (
                f"forecast the next {str(self.pred_len)} steps "
                f"given the previous {str(self.seq_len)} steps information"
            )

        report = report.strip() if report else ""
        report_block = f"Clinical report: {report} " if report else ""
        ...
        return (
            ...
            f"{report_block}"
            f"Input statistics: "
            f"min value {min_val_str}, "
            f"max value {max_val_str}, "
            f"median value {median_val_str}, "
            f"the trend of input is {trend_str}, "
            f"top 5 lags are : {lags_str}"
            f"<|end_prompt|>"
        )
    ##########################################################
    ...
    def classify(self, x_enc, report=None):
        ...
        for b in range(x_enc.shape[0]):   # iterates B * N
            ...
            prompt.append(self._build_prompt(
                ...
                median_val_str = str(medians[b].tolist()[0]),
        ...
        dec_out = self.dropout_cls(dec_out)
